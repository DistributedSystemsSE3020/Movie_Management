...
exports.additem = async(req,res) => {
    const {movieid,customerID,quantity,type,total} = req.body;

    try {
        //checking product already exists
        const checkItem = await Cart.findOne({movieid,customerID})
        if(checkItem)
            return res.status(409).json({message: "Item already exists"})

        //creating a new add item
        await Cart.create({movieid,customerID,quantity,type,total});
        //success message
...
exports.viewCart = async(req,res) => {
    ...
    try {
        ...
        const cart = await Cart.find({customerID,type}).populate(
            {path:'movieid', select:['name','price','description','total','imgUrl']});
        //success message
